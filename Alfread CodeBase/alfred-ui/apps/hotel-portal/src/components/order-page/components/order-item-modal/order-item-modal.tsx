								...
								{product?.modifiers?.map((modifier: any) => (
									<React.Fragment key={modifier?.id}>
										...
										{modifier?.multipleOptions ? (
											...
											<Radio.Group
												required={modifier?.requiredOptions}
												value={find(temporaryItem?.modifiers, {
													id: modifier?.id
				...
				>
					<Flex gap={isMobileOnly ? "0.2rem" : "sm"} align='center'>
						<ActionIcon
							...
							radius='lg'
							variant='transparent'
							disabled={temporaryItem?.quantity <= 1}
						>
							<IconCircleMinus
								size={30}
								color={temporaryItem?.quantity <= 1 ? "gray" : "black"}
								onClick={() => {
									if (temporaryItem?.quantity > 1) {
						...
						<StyledNumberInput
							...
							min={1}
							value={temporaryItem?.quantity}
							onChange={(value: any) => {
								if (value > 0) {
									changeTemporaryItemQuantity(value)
								}
							}}
						/>
					...
					<Flex gap={isMobileOnly ? "0.5rem" : "sm"}>
						<Flex align='center' columnGap={8}>
							<PriceAfterDiscount>
								{showPrice(
									calculateTotalPrice({ items: [temporaryItem] }).totalPrice
								)}
							</PriceAfterDiscount>
						</Flex>
						<StyledButton
