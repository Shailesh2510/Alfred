...
const RideCheckout = () => {
	...
	const { campaign_uid, short_code, merchantId } = router.query
	...
	const isPayLaterEnabled = validateScheduleRideTime(
		ride?.scheduledDate,
		...
		onSuccess: (result: any) => {
			if (result?.isCarmelTripCreated) {
				paymentInit({
					orderId: rideNonce,
					amount:
						paymentMethodType === PAYMENT_METHOD.PAY_LATER.value
							? 0
							: parseFloat(totalPrice) * 100,
					paymentMethodType: "card",
					clientName:
						checkoutForm.clientFirstName + " " + checkoutForm.clientLastName,
					clientNumber: `${checkoutForm?.clientNumber}`,
	...
	const form = useForm({
		...
		initialValues: {
			clientLastName: checkoutForm.clientLastName,
			clientFirstName: checkoutForm.clientFirstName,
			clientNumber: checkoutForm.clientNumber,
			clientEmail: checkoutForm.clientEmail
		},
		validate: {
	...
	useEffect(() => {
		if (paymentMethodType === PAYMENT_METHOD.PAY_LATER.value) {
		...
		usePaymentInit({
			onSuccess: (result: any) => {
				if (paymentMethodType === PAYMENT_METHOD.PAY_LATER.value) {
					...
					resetCheckoutFormValue()
					resetRideFormValue()
					router.push(`/ride/${rideNonce}?orderStatus=success`)
				} else {
					if (result?.data?.[0]?.clientSecret) {
						...
						if (campaign_uid && short_code) {
							createReferralRecord({
								...
								clientName:
									checkoutForm.clientFirstName +
									" " +
									checkoutForm.clientLastName,
								clientEmail: checkoutForm?.clientEmail,
	...
	const createTripUpdatePayload = (response: any) => {
		const firstSpaceIndex = checkoutForm?.clientNumber.indexOf(" ")
		const countryCode = checkoutForm?.clientNumber
			.slice(0, firstSpaceIndex)
			.replace("+", "")
		const number = checkoutForm?.clientNumber
		...
		return {
			...
			customerFirstName: checkoutForm?.clientFirstName,
			customerLastName: checkoutForm?.clientLastName,
			customerPhone: {
				countryCode: countryCode,
				number: number
			},
			emailAddr: checkoutForm?.clientEmail,
	...
	const paymentOptions = [
		...
		...(isPayLaterEnabled
			? [
					{
						label: PAYMENT_METHOD.PAY_LATER.label,
						value: PAYMENT_METHOD.PAY_LATER.value
					}
			  ]
			: [])
	]
		...
		<>
			...
			<RideCheckoutContainer>
				<Grid gutter={24} justify='center' align='center'>
					<Grid.Col xs={12} sm={8} md={6} lg={6} xl={4}>
									...
									<Grid.Col>
										<StyledTextInput
											...
											onChange={(clientFirstName: any) => {
												...
												setCheckoutFormValue({
													...checkoutForm,
													clientFirstName: clientFirstName?.target.value
												})
											}}
										/>
										<StyledTextInput
											...
											onChange={(clientLastName: any) => {
												...
												setCheckoutFormValue({
													...checkoutForm,
													clientLastName: clientLastName?.target.value
												})
											}}
										/>
							...
							{parseFloat(totalPrice) > 0.5 ? (
								<Accordion.Item value='payment-information'>
									...
									>
										...
										/>
										{paymentMethodType === PAYMENT_METHOD.CREDIT_CARD.value ? (
											...
											<PaylaterText>
												{`Pay Later sends a payment link via text and email, due
												one hour before the ride. Credit Card lets guests pay
												immediately.`}
											</PaylaterText>
										)}
									</Accordion.Panel>
