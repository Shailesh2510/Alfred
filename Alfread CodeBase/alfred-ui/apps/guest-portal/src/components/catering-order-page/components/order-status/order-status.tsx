...
const OrderStatus = ({ dispatchCart, cartState }: any) => {
	const router = useRouter()

	const { orderId, orderStatus } = router.query
					...
					<>
						{orderStatus === "success" && (
							<>
								...
								<OrderInfoLabel>
									In the meantime, if you have any questions, please feel free
									to reach us at{" "}
									<Anchor href='mailto:info@getalfred.com'>
										info@getalfred.com
									</Anchor>
								</OrderInfoLabel>
							</>
			...
			<Grid>
				...
				>
					<Flex align='center' mb={12}>
						<ActionIcon
							onClick={() => {
								router.push(`/catering/${currentOrder?.hotelWebCode}`)
							}}
							variant='transparent'
						>
							<IconArrowLeft />
